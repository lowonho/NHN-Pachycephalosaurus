...
기존 7개 물리 프로토타입에 기억 조각과 복구 진행도를 연결한다. 공통 시스템 도입 시 기존 물리와 맵을 유지했으며, 이후 1스테이지는 넓은 가속 코스와 벽 충돌 시간 페널티로 구체화했다. 세부 규칙은 `record-01-course.md`를 참조한다. 밈 아트와 나머지 위험 루트의 최종 난이도는 다음 작업이다.
...
| 스테이지 | 수집 주체 | 임시 배치 |
| --- | --- | --- |
| 가속 코스 | 공 | S자 코스 출구 오른쪽 위의 순환 경로 |
| 중력 타워 | 플레이어 | 마지막 발판 오른쪽 위, 추가 점프 후 공중 방향 전환으로 귀환 |
