...
| e10 | e10_numberDecode.js | acceleration, maxSpeed, jump, baseFriction, frictionLoss, minFriction |
...
- e10은 A/D 또는 좌우 방향키로 이동하고 Space로 점프하여 0~9 블록의 아랫면을 터치합니다. AI가 읽지 못한 네 자리 목표를 순서대로 입력하면 성공합니다. 방향을 새로 누를 때마다 바닥 마찰이 감소하며, 오답과 화면의 `전체 삭제` 버튼은 입력값만 비웁니다.
...
검증 범위: 10개 로딩, 랜덤 5개 중복 방지, 20.26초 종료, 성공 경로, 페널티 유지, 정지/전환 정리, 룰렛 당첨/꽝 판정, 숫자 블록 충돌·오답·전체 삭제·마찰 감소, 최고 기록, 결과의 3개 버튼, 실제 키보드와 축소된 모니터의 마우스 드래그. 화면 캡처는 `tests/.artifacts/`에 저장됩니다. 자동 입력 성공 경로는 플레이 난이도를 인간 대상으로 검증한 결과를 의미하지 않으며 세부 밸런스는 플레이테스트 후 조정합니다.
