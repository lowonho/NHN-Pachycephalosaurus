...
- e2는 A/D 좌우, W 체공/S 급강하, Space 착지 후 점프입니다. 왁뿌볼의 껍질에 점프마다 균열과 파손이 누적됩니다. 점프 속도는 740에서 매번 90%로 줄어 11번째 점프부터 최저 280이 됩니다. 초반 높은 점프는 유지하면서 중후반에도 높이가 계속 변합니다. 천장은 열려 있고 높은 점프는 카메라가 따라갑니다. 발판 14개 중 파랑 3개는 공을 운반하는 승강 발판(초반 진폭 22px, 후반 10px), 주황 3개는 착지 후 0.55초 뒤 무너졌다가 1.4초 뒤 복구되는 발판입니다. 초반 간격은 105~115px, 6번째 발판 이후는 65px이며 착지 면을 넓히고 오르막 높이를 줄였습니다. 후반은 최저 점프력에서도 W 없이 건널 수 있게 설계하고, 승강 발판 시작 위상 4가지와 100ms 입력 간격으로 검증합니다. 공중 연타는 파손을 추가하지 않습니다. 사망 후에도 파손과 점프력을 유지하며, 붕괴 발판은 체크포인트로 저장하지 않습니다. 결과의 다시하기는 공과 지형을 초기화합니다.
