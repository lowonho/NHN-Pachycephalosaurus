...
| e4 | e4_accelerationDash.js | speed, gain, maxSpeed, radius, wallPenalty, grid, timeLimit |
...
| e8 | e8_webSwing.js | speed, acceleration, maxSpeed, jump, gravity, hookRange, roofWidths, gapWidths |
| e9 | e9_iceCurling.js | friction, decay, minFriction, force, targetRadius |

## 공통 흐름

- 메인 → 오프닝 → 중복 없이 뽑은 랜덤 5개 선택 → 게임 → 결과.
- 기본 제한시간은 20.26초이며 큰 미로인 e4는 90초입니다. 기존 누적 2:23 실패 제한 대신 게임마다 독립된 타이머를 사용합니다.
...
- e4는 19×7칸(876×312px)의 랜덤 미로를 방향키/WASD로 자유롭게 탐색합니다. 갈림길·막다른 길·순환 통로, 한 화면에 보이는 출발점과 도착점를 제공합니다. 통로 폭은 84~88px, 벽 두께는 12px입니다. 이동 속도는 180→520px/s로 약 0.52초 만에 가속하며, 속도선과 긴 잔상으로 가속을 표시하고 키를 놓으면 정지합니다. 기본 90초, 벽 접촉마다 1초 차감(접촉 유지 중 중복 없음), 재도전 시 초기화합니다. 초록 출구에 도착하면 성공이며 QA 제한시간은 우선 적용됩니다.
...
- e8은 코인 없이 옥상 9개를 건너는 거미줄 액션 러너입니다. Space/클릭으로 점프하고 공중에서 다시 누르고 있으면 앞쪽 엠파이어 스테이트 빌딩 첨탑에 줄을 겁니다. 놓으면 접선 방향의 상승 속도를 유지하며 날아갑니다. 줄은 최대 620px 앞의 연결 가능한 첨탑에 걸리고, 팽팽할 때 원호를 따라 이동합니다. 자동 전진 속도는 320px/s에서 매초 55씩 올라 최대 1120px/s(3.5배), 건물 사이 간격은 350~850px입니다. 빨라질수록 시야도 넓어집니다. 옥상 옆면 충돌/추락은 마지막 착지 건물로 돌아가며 시간과 속도는 유지합니다. 마지막 옥상의 깃발까지 도착해야 성공하고 제한시간 생존만으로는 성공하지 않습니다. 기본 무추락 경로는 약 15.5초, 제한시간은 20.26초입니다. 조작을 50~83ms 간격으로 판단하는 여러 성공 경로를 검증했으며 늦은 줄 해제는 추락할 수 있습니다. 그림 교체는 [e8 에셋 가이드](../../../assets/minigames/e8/README.md)를 참고하세요.
