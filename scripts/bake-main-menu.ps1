# 메인 화면 — 배경 일러스트와 캐릭터 원본 png 를 화면이 쓰는 webp 로 굽는다.
#
# 원본 두 장은 assets/images/ui/Main 에 그대로 두고(다시 구울 때 필요하다),
# 같은 폴더에 bg.webp · character.webp 를 만든다. css/main-menu.css 가 쓰는 건 webp 쪽이다.
#
# 하는 일은 셋이다.
#   1) 캐릭터는 투명 여백을 잘라낸다. 잘라 두면 CSS 에서 "바닥에 붙이고 폭 얼마" 로
#      자리를 잡을 수 있다 — 여백이 남아 있으면 여백만큼 떠 보인다.
#   2) 캐릭터 가장자리 색을 투명 쪽으로 번지게 한다. webp 손실 압축이 머리카락 실루엣에
#      흰 테두리를 남기지 않는다. 배경은 알파가 없어 건너뛴다.
#   3) 긴 변을 MaxSide 로 줄여 굽는다. 원본보다 키우지는 않는다(키워 봐야 흐려질 뿐이다).
#
...
param(
  ...
  [string]$Chrome = "",
  # 배경 webp 품질. 1920 폭으로 깔리는 큰 그림이라 0.82 면 눈으로 구분되지 않는다.
  [double]$BackgroundQuality = 0.82,
  # 캐릭터 webp 품질. 얼굴이 크게 보이므로 배경보다 높게 잡는다.
  [double]$CharacterQuality = 0.9
)

$ErrorActionPreference = "Stop"
$repoRoot = Split-Path -Parent $PSScriptRoot
$artRoot = Join-Path $repoRoot "assets/images/ui/Main"
$tempRoot = Join-Path ([IO.Path]::GetTempPath()) ("main-menu-bake-" + [Guid]::NewGuid().ToString("N").Substring(0, 8))
New-Item -ItemType Directory -Path $tempRoot -Force | Out-Null

# 구울 목록. Trim 이 켜진 쪽만 투명 여백을 자르고 색을 번지게 한다.
$roles = @(
  [pscustomobject]@{ Key = "bg";        Note = "배경 아카이브 광장"; Trim = $false; MaxSide = 1920; Quality = $BackgroundQuality },
  [pscustomobject]@{ Key = "character"; Note = "메인 캐릭터";        Trim = $true;  MaxSide = 1400; Quality = $CharacterQuality }
)
...
public class MainMenuArt
{
    ...
    /* 한 장을 다듬어 pngPath 에 쓴다. 돌려주는 문자열은 "원본가로 원본세로 결과가로 결과세로". */
    public static string Bake(string sourcePath, string pngPath, int maxSide, bool trim)
    {
        int width, height;
        byte[] source = ReadBgra(sourcePath, out width, out height);

        int x0 = 0, y0 = 0, tw = width, th = height;
        if (trim)
        {
            int x1 = -1, y1 = -1;
            x0 = width; y0 = height;
        ...
        byte[] baked = new byte[tw * th * 4];
        for (int y = 0; y < th; y++)
            Buffer.BlockCopy(source, ((y + y0) * width + x0) * 4, baked, y * tw * 4, tw * 4);
        if (trim) Bleed(baked, tw, th);

        // 원본보다 키우지는 않는다 — 없는 정보를 늘려 봐야 흐려지고 파일만 커진다.
        double scale = Math.Min(1.0, (double)maxSide / Math.Max(tw, th));
        int ow = Math.Max(1, (int)Math.Round(tw * scale));
        int oh = Math.Max(1, (int)Math.Round(th * scale));
        using (Bitmap src = ToBitmap(baked, tw, th))
...
foreach ($role in $roles) {
  ...
  $size = [MainMenuArt]::Bake($sourcePath, $tempPng, $role.MaxSide, $role.Trim).Split(" ")
  $baked += [pscustomobject]@{
    Key = $role.Key; Note = $role.Note; Png = $tempPng; Source = $sourcePath; Quality = $role.Quality
...
'@
  ...
  foreach ($record in $baked) {
    ...
    $quality = $record.Quality.ToString([Globalization.CultureInfo]::InvariantCulture)
    ...
    "{0,-10} {1,-16} {2,4}x{3,-4} <- {4,4}x{5,-5} png {6,9:N0} -> webp {7,8:N0}" -f `
